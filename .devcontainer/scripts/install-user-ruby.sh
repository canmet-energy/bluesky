...
echo "ðŸ’Ž Installing Ruby via rbenv for user $(whoami)..."
...
if [[ "$RUBY_VERSION_INPUT" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    echo "âŒ Invalid Ruby version format: '$RUBY_VERSION_INPUT' (expected MAJOR.MINOR.PATCH)" >&2
    exit 2
fi
...
if [ -d "$RUBY_BUILD_DIR/.git" ]; then
    ...
    echo "ðŸ“¥ Installing ruby-build plugin..."
...
if [ ! -d "$RBENV_UPDATE_DIR/.git" ]; then
    echo "ðŸ“¥ Installing rbenv-update plugin..."
...
if rbenv versions --bare | grep -q "^${RUBY_VERSION}$"; then
    ...
    if MAKE_OPTS="-j${MAKE_JOBS}" RUBY_CONFIGURE_OPTS="--disable-install-doc" rbenv install "$RUBY_VERSION" --verbose; then
        ...
        echo "âŒ Ruby installation failed. Check build dependencies." >&2
...
echo "ðŸ”§ Setting Ruby ${RUBY_VERSION} as global default..."
...
if ruby --version; then
    ...
    echo "âŒ Ruby verification failed" >&2
    exit 1
fi

echo ""
echo "ðŸŽ‰ Ruby installation complete! (Gemfile management moved to post-create script)"
