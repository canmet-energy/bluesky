...
class NECBVectorIndexer:
    ...
    def _build_vintage_index(self, vintage: str, force_rebuild: bool) -> int:
        ...
        if not documents:
            logger.info(f"No documents for NECB {vintage}, skipping")
            return 0
    ...
    def _load_documents(self, vintage: str) -> List[NECBDocument]:
        ...
        cursor.execute(
            """
            SELECT id, article_number, title, full_text, page_start
            FROM necb_articles
            WHERE vintage = ?
        """,
            (vintage,),
        ...
        for row in cursor.fetchall():
            article_id, article_num, title, content, page_num = row
            ...
            doc = NECBDocument(
                id=f"{vintage}_section_{article_id}",
                content=f"Section {article_num}: {title}\n\n{content}",
                metadata={
                    "vintage": vintage,
        ...
        # Load figures
        # Check if ai_description column exists
        cursor.execute("PRAGMA table_info(necb_figures)")
        columns = {row[1] for row in cursor.fetchall()}
        has_ai_description = "ai_description" in columns

        if has_ai_description:
            cursor.execute(
                """
                SELECT id, label, caption, page, image_path, image_type, width, height, ai_description
                FROM necb_figures
                WHERE vintage = ?
            """,
                (vintage,),
            ...
            cursor.execute(
                """
                SELECT id, label, caption, page, image_path, image_type, width, height
                FROM necb_figures
                WHERE vintage = ?
            """,
                (vintage,),
        ...
        for row in cursor.fetchall():
            if has_ai_description:
                figure_id, label, caption, page_num, image_path, image_type, width, height, ai_description = row
            else:
                figure_id, label, caption, page_num, image_path, image_type, width, height = row
                ai_description = None

            # Use AI description if available, otherwise use caption
            description = ai_description or caption

            # Skip figures without any description
            if not description or not description.strip():
                continue

            # Format figure as text
            figure_text = f"Figure {label}\n\n"

            if ai_description:
                # Prefer detailed AI description
                figure_text += f"Description: {ai_description}"
                if caption and caption != ai_description:
                    # Include original caption for context
                    figure_text += f"\n\nOriginal caption: {caption}"
            elif caption:
                # Fall back to caption only
                figure_text += f"Caption: {caption}"
            ...
            doc = NECBDocument(
                id=f"{vintage}_figure_{figure_id}",
                content=figure_text,
                metadata={
                    "vintage": vintage,
                    "content_type": "figure",
        ...
        logger.info(
            ...
            f"{len([d for d in documents if d.metadata['content_type'] == 'figure'])} figures"
        )
...
def main():
    ...
    if not args.db.exists():
        ...
        print("Run: python -m bluesky.necb.build")
        sys.exit(1)
