...
def get_optimal_embedding_model() -> Tuple[SentenceTransformer, dict]:
    ...
    try:
        ...
        if torch.cuda.is_available():
            # Find a compatible GPU (compute capability >= 7.0)
            # PyTorch requires sm_70+ for current versions
            compatible_gpu = None
            ...
            for i in range(device_count):
                ...
                logger.info(f"  GPU {i}: {gpu_name} (compute capability {compute_cap})")

                if compute_cap >= 7.0 and compatible_gpu is None:
                    compatible_gpu = i
                    logger.info(f"  → Selected GPU {i}: {gpu_name}")

            if compatible_gpu is not None:
                # Use the compatible GPU
                device = f"cuda:{compatible_gpu}"
                torch.cuda.set_device(compatible_gpu)
                model_name = "intfloat/e5-large-v2"
                dimensions = 1024
                gpu_name = torch.cuda.get_device_name(compatible_gpu)
                gpu_memory = torch.cuda.get_device_properties(compatible_gpu).total_memory / 1e9
                ...
                config_info = {
                    ...
                    "gpu_index": compatible_gpu,
                }
            else:
                # No compatible GPU found, fall back to CPU
                logger.warning("No compatible GPU found (need compute capability >= 7.0)")
